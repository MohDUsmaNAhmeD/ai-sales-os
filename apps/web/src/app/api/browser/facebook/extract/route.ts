...
export async function POST() {
  try {
    const result = await extractCookiesForPlatform("FACEBOOK");

    if (result.cookieCount === 0) {
      return NextResponse.json(
        { error: "No Facebook cookies found. Make sure you are logged in." },
        { status: 400 }
      );
    }

    await saveCookiesForPlatform("FACEBOOK", result.cookies);

    const existing = await prisma.connectorState.findFirst({
      where: { platform: "FACEBOOK", userId: "default" },
    });
    ...
    return NextResponse.json({
      ...
      message: `Extracted ${result.cookieCount} Facebook cookies from ${result.domains.length} domains`,
    });
  } catch (error) {
    return NextResponse.json(
